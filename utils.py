...
def test_single_volume(image, label, text_batch, net, classes, multimask_output, patch_size=[256, 256], input_size=[224, 224],
...
def train_single_volume(image, label, text, net, classes, multimask_output, patch_size=[256, 256], input_size=[224, 224],
                       test_save_path=None, case=None, z_spacing=1):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    if len(image.shape) == 3:
        prediction = np.zeros_like(label)
        ...
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():
                ...
                outputs = net(inputs, text, multimask_output, patch_size[0])
