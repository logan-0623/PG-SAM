...
class MLP(nn.Module):
    ...
    def forward(self, x):
        for i, layer in enumerate(self.layers):
            x = self.dropout(F.relu(x)) if i < self.num_layers - 1 else layer(x)
...
class MaskDecoder2_224(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.output_upscaling = nn.Sequential(
            ...
            nn.Conv2d(transformer_dim, transformer_dim // 4, 3, padding=1),
            LayerNorm2d(transformer_dim // 4),
            activation(),
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
        ...
        self.fusion_conv = nn.Sequential(
            nn.Conv2d(transformer_dim * 2, transformer_dim, kernel_size=1),
    ...
    def predict_masks(
        ...
        if mode =='train':
            msk_feat = torch.nn.Dropout(p=0.2, inplace=True)(msk_feat)
        ...
        src = self.fusion_conv(torch.cat([src, src * msk_feat], dim=1))  # 改进后的特征融合
